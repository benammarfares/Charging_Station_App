...
{
    QSqlQuery query;
    query.prepare("SELECT ID_SESSION FROM VOITURE WHERE ID_SESION=:ID_SESION");
...
bool session::insert()
{
    ...
    query.prepare("INSERT INTO SESION (ID_SESSION, MODEE, NIV_BATTERIE,VITESSE, HAPPY_HOUR, PRIX) "
...
bool session::delete_session(int id1) {
    ...
    query.prepare("Delete from SESION where ID_SESSION= :id1");
...
bool session::update(QString id){
    QSqlQuery query;
        query.prepare("UPDATE SESION SET MODEE=:MODEE, NIV_BATTERIE=:NIV_BATTERIE, VITESSE=:VITESSE, HAPPY_HOUR=:HAPPY_HOUR, PRIX=:PRIX WHERE ID_SESSION=:id");
...
{
    QSqlQueryModel * model=new QSqlQueryModel();
    model->setQuery("SELECT * from SESION WHERE ID_SESSION LIKE '%"+ch+"%'");
